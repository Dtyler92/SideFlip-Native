const focusEventOwners = new WeakMap()

export function claimFocusEvent(event, owner) {
  const key = event?.nativeEvent && typeof event.nativeEvent === 'object' ? event.nativeEvent : event
  if (!key || (typeof key !== 'object' && typeof key !== 'function')) return true
  const existingOwner = focusEventOwners.get(key)
  if (existingOwner && existingOwner !== owner) return false
  focusEventOwners.set(key, owner)
  return true
}
