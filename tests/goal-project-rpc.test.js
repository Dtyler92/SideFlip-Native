...
test('goal-linked project creation can use available goal funds atomically', () => {
  ...
  assert.match(value, /mutationIdForPayload\(projectMutationAttempt\.current, rpcPayload\)/)
  assert.match(value, /p_mutation_id:\s*mutationId/)
  assert.match(value, /resetMutationAttemptState\(projectMutationAttempt\.current\)/)
