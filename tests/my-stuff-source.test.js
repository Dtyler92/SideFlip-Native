...
test('My Stuff uses only its three separate tables and fixed creation/completion RPCs', () => {
  ...
  assert.match(client, /from\('my_stuff_schedules'\)/)
  ...
  assert.match(client, /rpc\('complete_my_stuff_maintenance'/)
  assert.doesNotMatch(client, /from\('(projects|expenses|trade_up_goals|goal_ledger)'\)/)
...
test('all My Stuff forms are keyboard-safe and support every maintenance mode and history', () => {
  ...
  assert.match(detail, /completeMyStuffMaintenance/)
})
...
test('maintenance retries retain one mutation key and block synchronous double submits', () => {
  const detail = source('src/screens/MyStuffDetailScreen.js')
  assert.match(detail, /completionMutationId=useRef\(null\)/)
  assert.match(detail, /completionInFlight=useRef\(false\)/)
  assert.match(detail, /if\(completionInFlight\.current\)return/)
  assert.match(detail, /mutationId=completionMutationId\.current\|\|/)
...
test('schedule retries retain one mutation key and block synchronous double submits', () => {
  const detail = source('src/screens/MyStuffDetailScreen.js')
  assert.match(detail, /scheduleMutationId=useRef\(null\)/)
  assert.match(detail, /scheduleInFlight=useRef\(false\)/)
  assert.match(detail, /if\(scheduleInFlight\.current\)return/)
  assert.match(detail, /mutationId=scheduleMutationId\.current\|\|/)
})
...
test('retained history readings do not depend on a surviving schedule', () => {
  ...
  assert.match(detail, /validateMaintenanceCompletion/)
})
