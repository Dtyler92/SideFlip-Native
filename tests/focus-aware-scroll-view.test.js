...
test('the nearest nested scroll view exclusively owns a bubbled focus event', () => {
  const event = { nativeEvent: { target: 42 } }
  const inner = {}
  const outer = {}
  assert.equal(claimFocusEvent(event, inner), true)
  assert.equal(claimFocusEvent(event, outer), false)
  assert.equal(claimFocusEvent(event, inner), true)
})
